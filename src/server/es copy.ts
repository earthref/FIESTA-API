...
// Search private contributions
async function esGetPrivateTSV({
	repository,
	contributor,
	id
}: {
	...
	return createTSVfile(resp.body.hits.hits[0]._source.contribution, model30)
}
export { esGetPrivateTSV };

function _getOrderedColumnList(tableName)
{
	//Find the index that holds the object which represents a given table
	for(const orderedTableIdx in orderedModel) {
		if (orderedModel[orderedTableIdx][tableName]) {
			return orderedModel[orderedTableIdx][tableName];
		}
	}
};

function createExtendedHeadersData(tableName, jsonToExport, model) {
	...
	const labelHeaderArray = [];
	const columnTypeOrUnitHeaderArray = [];
	const columnHeaderArray = [];
	const orderedColumnArray = _getOrderedColumnList(tableName);

	//loop through ordered model columns for this table, see if json has data from that column
	for (const orderedColumnIdx in orderedColumnArray) {

		const potentialColumnToAdd = orderedColumnArray[orderedColumnIdx];
		...
		if (tableName.toLowerCase() === 'measurements') {
			for(const jsonRowIdx in jsonToExport[tableName].columns) {
				...
				if (jsonToExport[tableName].columns[jsonRowIdx].toLowerCase() === potentialColumnToAdd.toLowerCase() &&
					...
					const labelToAdd = model.tables[tableName].columns[potentialColumnToAdd].label;
					labelHeaderArray.push(labelToAdd);

					// Append the group name if it's not the same as the previous column.
					const currentGroupToAdd = model.tables[tableName].columns[potentialColumnToAdd].group;
					groupHeaderArray.push((previousGroupFound != currentGroupToAdd ? currentGroupToAdd : ''));
					previousGroupFound = currentGroupToAdd;

					columnTypeOrUnitHeaderArray.push(getColumnTypeOrUnitString(tableName,potentialColumnToAdd));
				}
			}
		}
		else {
			for(let jsonRowIdx in jsonToExport[tableName]) {
				...
				if (jsonToExport[tableName][jsonRowIdx][potentialColumnToAdd] && //if this ordered column is found in the json file
					...
					let labelToAdd = model.tables[tableName].columns[potentialColumnToAdd].label;
					labelHeaderArray.push(labelToAdd);

					// Append the group name if it's not the same as the previous column.
					let currentGroupToAdd = model.tables[tableName].columns[potentialColumnToAdd].group;
					groupHeaderArray.push((previousGroupFound != currentGroupToAdd ? currentGroupToAdd : ''));
					previousGroupFound = currentGroupToAdd;
...
function generateExtendedHeaderTSV(tableName, jsonToExport)
{
	let extendedHeader = '';
	const extraHeaderData = createExtendedHeadersData(tableName, jsonToExport/*, orderedColumnArray*/);
...
function createTSVfile(jsonToExport, toExtendedText): string {
	...
	{
		let tableName = Object.getOwnPropertyNames(orderedModel[orderedTableIdx])[0];//loop through the tables in the model
		if(tableName && jsonToExport[tableName])//if the current table exists in the json to translate, add it to the output file
		{
			...
			if (toExtendedText)
			{
				...
				text += generateExtendedHeaderTSV(tableName, jsonToExport/*, orderedColumnArray*/)
			}
			...
			if (tableName.toLowerCase() === 'measurements') {
				...
				for (let jsonRowsIdx in jsonToExport[tableName]) {
					for (let colNameIdx in orderedListOfColumnsAddedToHeader) {
						...
						text += handleSpecialCases(dataToAdd, tableName, columnName);
...
function handleSpecialCases(value, tableName, columnName) {
	try {
		const type = models[_.last(versions)].tables[tableName].columns[columnName].type;

		if (type === 'Dictionary' && _.isPlainObject(value))
			value = _.keys(value).map(key => `${escapeColons(key)}[${escapeColons(value[key])}]`).join(':');

		else if (type === 'Matrix' && _.isArray(value))
			value = value.map(row => row.map(col => escapeColons(col)).join(':')).join(';');

		else if (type === 'List' && _.isArray(value))
			value = value.map(item => escapeColons(item)).join(':');
	} catch (e) {}
	return value;
}

function escapeColons(value) {
	value = _.trim(value);
