...
describe('FIESTA API v0 Download Tests', () => {
	...
	test(
		'GET /v0/MagIC/download should return status 400 - ' +
			'either a contribution ID or query parameters are required',
		async () => {
			const res = await client.get('/v0/MagIC/download');
	...
	test(
		'POST /v0/MagIC/download should return status 404 - ' +
			'POST is not defined for the download path',
		async () => {
			const res = await client.post('/v0/MagIC/download');
			expect(res.status).toBe(404);
		}
	);

	test(
		'GET /v0/MagIC/download?id=1 should return status 204 - ' +
			'there are no public contributions with this contribution ID',
		async () => {
			const res = await client.get('/v0/MagIC/download?id=1');
	...
	test(
		...
		async () => {
			const latestRes = await client.get(
				'/v0/MagIC/search/contributions?n_max_rows=1'
			);
			const latestCID = latestRes.data.results[0].id;
			const res = await client.get(`/v0/MagIC/download?id=${latestCID}`);
	...
	test(
		...
		async () => {
			const res = await client.get('/v0/MagIC/download?id=1a');
			expect(res.status).toBe(400);
			expect(res.data).toHaveProperty('errors');
		}
	);
	...
	test(
		'GET /v0/MagIC/download?doi=10.1029/JZ072I012P03247 should return status 200 - ' +
			'there are public contributions with this reference DOI',
		async () => {
			const res = await client.get(
				'/v0/MagIC/download?doi=10.1029/JZ072I012P03247'
			);
			expect(res.status).toBe(200);
		}
	);
	...
	test(
		'GET /v0/MagIC/download?id=1&id=16595&id=16761 should return status 200 - ' +
			'there are public contributions for the second and third contribution IDs',
		async () => {
			const res = await client.get(
				'/v0/MagIC/download?id=1&id=16595&id=16761',
				{ responseType: 'stream' }
			);
			expect(res.status).toBe(200);
			const paths: string[] = [];
